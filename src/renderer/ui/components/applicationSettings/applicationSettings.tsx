    ...
    if (appConfig) {
      return (
        ...
        >
          <div
            className={clsx(
              settingsScreenStyles["settings-screen-panel"],
              styles["settings-editor"]
            )}
          >
            <SettingBlock
            ...
            >
              ...
              >
                <select
                  id="mem-saver-inactive"
                  value={appConfig.memSaver.workflowInactiveAfter}
                  onChange={(e) =>
                    updateSettings({
                      ...appConfig,
                      memSaver: {
              ...
              >
                <select
                  id="mem-saver-activate-on-project"
                  value={convertBoolToStr(
                    appConfig.memSaver.activateWorkflowsOnProjectSwitch
                  )}
                  onChange={(e) =>
                    updateSettings({
                      ...appConfig,
                      memSaver: {
                        ...appConfig.memSaver,
                        activateWorkflowsOnProjectSwitch: convertStrToBool(
                          e.target.value
                        ),
                      },
                    })
                ...
                >
                  {activateOnProjectSwitchOptions.map((item) => (
                    <option
                      key={convertBoolToStr(item.val)}
                      value={convertBoolToStr(item.val)}
                    >
                      {item.name}
                    </option>
                  ))}
            ...
            <SettingBlock
              titleForId="mongo-uri"
              title="MongoDB URI"
              moreInfo="The connection string used to connect to your MongoDB instance."
            >
              <input
                id="mongo-uri"
                type="text"
                value={appConfig.mongoDB.uri}
                onChange={(e) =>
                  updateSettings({
                    ...appConfig,
                    mongoDB: {
            ...
            <SettingBlock
              titleForId="mongo-db-name"
              title="MongoDB Database Name"
              moreInfo="The name of the database to use within your MongoDB instance."
            >
              <input
                id="mongo-db-name"
                type="text"
                value={appConfig.mongoDB.databaseName}
                onChange={(e) =>
                  updateSettings({
                    ...appConfig,
                    mongoDB: {
...
export type ApplicationSettingsComponent = ReturnType<
  typeof createApplicationSettingsComponent
>;
